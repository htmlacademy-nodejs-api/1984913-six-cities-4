...
export default class CommentController extends Controller {
  constructor(
    ...
    this.logger.info(LoggerInfoMessage.RegisterRoute.concat('CommentController'));

    this.addRoute({path: ControllerRoute.Offer, method: HttpMethod.Post, handler: this.create});
    this.addRoute({path: ControllerRoute.Offer, method: HttpMethod.Get, handler: this.index});
  }
  ...
  public async create(
    {body, params}: Request<ParamsOfferDetails, UnknownRecord, CreateCommentDTO>,
    res: Response,
  ): Promise<void> {
    const offerId = params.offerId || '';
